...
class ACGAN():
  def __init__(self,latent_size):
    ...
    adam_lr = 0.0002
    adam_beta_1 = 0.5
    self.optimizer = Adam(lr=adam_lr, beta_1=adam_beta_1)
                    
    self.G = self.build_generator()
    self.G.summary()
    
    self.D = self.build_discriminator()
    self.D.compile(optimizer=self.optimizer,
  ...
  def save(self,epoch):
    ...
    self.G.save('generator_epoch_{0:03d}.h5'.format(epoch))
    self.D.save('discriminator_epoch_{0:03d}.h5'.format(epoch))
    self.combined.save('combiend_epoch_{0:03d}.h5'.format(epoch))
  ...
  def call(self):
    ...
    # get a fake image
    fake = self.G([latent, image_class])

    # we only want to be able to train generation for the combined model
    self.D.trainable = False
    fake, aux = self.D(fake)
    self.combined = Model([latent, image_class], [fake, aux])
    ...
    for epoch in range(1, epochs + 1):
        ...
        # see if the discriminator can figure itself out...
        discriminator_test_loss = self.D.evaluate(
        ...
        print('{0:<22s} | {1:4s} | {2:15s} | {3:5s}'.format(
            'component', *self.D.metrics_names))
        print('-' * 65)
        ...
        Image.fromarray(img).save(
            'epoch_{0:03d}_generated.png'.format(epoch))
