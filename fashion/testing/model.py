...
class Genertor():
    def __init__(self,inputs, is_train=True, reuse=False):
        self.train  = is_train
        self.reuse  = reuse
        self.model = self.G_build(inputs)


    def G_build(self,inputs):
        image_size = 28 # the size of image in fashion_mnist dataset
        s16 = image_size // 16
...
class Discriminator():
    def __init__(self,inputs, is_train=True, reuse=False):
        self.train  = is_train
        self.reuse  = reuse
        self.model  = self.D_build(inputs)


    def D_build(self,inputs):
        ...
        with tf.variable_scope("discriminator", reuse=self.reuse):
            ...
            net_h1 = BatchNormLayer(net_h1, act=tf.nn.leaky_relu,
                    is_train=self.train, gamma_init=gamma_init, name='d/h1/batch_norm')
            ...
            net_h2 = BatchNormLayer(net_h2, act=tf.nn.leaky_relu,
                    is_train=self.train, gamma_init=gamma_init, name='d/h2/batch_norm')
            ...
            net_h3 = BatchNormLayer(net_h3, act=tf.nn.leaky_relu,
                    is_train=self.train, gamma_init=gamma_init, name='d/h3/batch_norm')
